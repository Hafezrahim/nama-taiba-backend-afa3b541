...
const ProfileCard = ({ user, t }: { user: any; t: (en: string, ar: string) => string }) => {
  const { data: profile } = useQuery({
    queryKey: ['marketer-sidebar-profile', user?.id],
    queryFn: async () => {
...
export const MarketerLayout = () => {
  ...
  useEffect(() => {
    if (!loading) {
      if (!user) {
        ...
        // Redirect to appropriate dashboard if they are not an approved marketer
  ...
  if (loading || !user || userRole !== 'marketer') return null;

  const links = [
    { nameEn: 'Dashboard', nameAr: 'لوحة القيادة', icon: LayoutDashboard, path: '/marketer' },
    { nameEn: 'Quotations', nameAr: 'عروض الأسعار', icon: FileText, path: '/marketer/quotations' },
    { nameEn: 'Confirmed', nameAr: 'المؤكدة', icon: CheckCircle, path: '/marketer/confirmed' },
    { nameEn: 'Profile', nameAr: 'الملف الشخصي', icon: User, path: '/marketer/profile' },
  ];
              ...
              <nav className="p-3 space-y-1">
                {links.map((link) => {
                  const isActive =
                    ...
                    (link.path !== '/marketer' && location.pathname.startsWith(link.path));
                  return (
                    <Link
                      key={link.path}
                      to={link.path}
